...
#     return pd.read_csv("data/action_areas_analysis_results.csv")
...
#       st.bar_chart(df_actions_count_percentage, x='actions number', y='Pourcentage', use_container_width=True,x_label="Actions aeras")
...
#         # Assurer que 'actions number' est entier
#         actions_clean['Action Area'] = actions_clean['Action Area'].astype(int)
...
#         entity_actionss = filtered_data.groupby('Entity')['Action Area'].apply(list)

#         for actionss in entity_actionss:
#             unique_actionss = sorted(set(actionss))
#             for pair in combinations(unique_actionss, 2):
...
# with tab2:
#     st.markdown("## 📋 Résumé des Analyses des top 5 actions")
...
#     actions_grouped = actions_filtered.groupby("Action Area")  # `actions` est le DataFrame chargé
#     contents = [
...
#                 status_text.text(f"Résumé généré pour {i + 1}/{total_steps} actionss.")
...
#         st.subheader("Résumé par actions")
#         for actions_number, summary in zip(actions_filtered.groupby("actions number").groups.keys(), st.session_state["summaries"]):
#             st.write(f"**actions {int(actions_number)}** : {summary}")
